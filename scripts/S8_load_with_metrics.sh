#!/usr/bin/env bash
# S8 — CPU iso-load with server-side resource metrics (wraps S7_load.js + Prometheus)
#
# Same harness as S7_load_with_metrics.sh (which wraps the IO-bound S6_load.js),
# pointed at the CPU-bound /compute endpoint instead. Runs the k6 test N times
# and, for each replicate, queries Prometheus over EXACTLY the k6 measured window
# (warm-up excluded) for CPU and memory.
#
# Why a wrapper: S7_load.js rewrites s7_<framework>.csv on every run, so N runs
...
# CPU and memory — and on a pure-compute endpoint that warm-up effect is the
# single largest confound, since the JVM's first thousand requests run in the
# interpreter while a native binary is at full speed from request one.
#
# Emits: framework,run,rps,iterations,p50_ms,p95_ms,p99_ms,err_pct,achieved_rps,
#        srv_p50_ms,srv_p95_ms,cpu_avg_cores,cpu_max_cores,mem_avg_mib,mem_max_mib,pod,k6_exit
#
# Usage: ./S8_load_with_metrics.sh <url-prefix> <deployment> [N]
set -euo pipefail
...
ENDPOINT="${ENDPOINT:-/compute}"
RPS="${RPS:-10}"
ITERATIONS="${ITERATIONS:-1000000}"    # per-request work size; keep identical across frameworks
...
OUT="${OUT:-s8_compute_metrics_${FRAMEWORK}.csv}"
...
# which would otherwise abort the whole test under `set -e`. A dead forward is
...
wait_ingress_ready() {
  local url="${TARGET}/${FRAMEWORK}${ENDPOINT}?iterations=${ITERATIONS}"
...
[[ -f "$OUT" ]] || echo "framework,run,rps,iterations,p50_ms,p95_ms,p99_ms,err_pct,achieved_rps,srv_p50_ms,srv_p95_ms,cpu_avg_cores,cpu_max_cores,mem_avg_mib,mem_max_mib,pod,k6_exit" > "$OUT"

echo "=== S8: $FRAMEWORK ($DEPLOY), $N runs, ${RPS} RPS x ${ITERATIONS} iterations, ${WARMUP_S}s warmup + ${MEASURE_S}s measured ==="
echo "    target: ${TARGET}/${FRAMEWORK}${ENDPOINT}   restart-between-runs: $RESTART"

K6_CSV="s7_${FRAMEWORK//[^A-Za-z0-9._-]/_}.csv"
...
for i in $(seq 1 "$N"); do
  ...
  k6 run --quiet \
     ...
     -e RPS="$RPS" -e ITERATIONS="$ITERATIONS" \
     -e WARMUP_S="$WARMUP_S" -e MEASURE_S="$MEASURE_S" \
     S7_load.js
  K6_EXIT=$?
  set -e
  ...
  # On /compute the pod is expected to sit well under its 1 CPU limit; at the
  # limit the measurement stops being "how fast does this runtime compute" and
  # becomes "how long does the scheduler make it wait".
  awk -v c="$CPU_MAX" 'BEGIN { if (c != "NaN" && c+0 > 0.9)
    print "  WARNING: cpu_max " c " cores is at the 1 CPU limit - request latency includes throttling." > "/dev/stderr" }'

  if [[ -f "$K6_CSV" ]]; then
    ROW=$(tail -n1 "$K6_CSV")
    IFS=, read -r _f _rps _it P50 P95 P99 ERR ARPS SRV50 SRV95 <<< "$ROW"
  else
    echo "WARNING: $K6_CSV missing — k6 produced no summary" >&2
    P50=NaN; P95=NaN; P99=NaN; ERR=NaN; ARPS=NaN; SRV50=NaN; SRV95=NaN
  fi

  echo "$FRAMEWORK,$i,$RPS,$ITERATIONS,$P50,$P95,$P99,$ERR,$ARPS,$SRV50,$SRV95,$CPU_AVG,$CPU_MAX,$MEM_AVG,$MEM_MAX,$POD,$K6_EXIT" | tee -a "$OUT"
done

echo
echo "Wrote $OUT. Next:"
echo "  python3 S5_stats.py $OUT --group framework --value p95_ms"
echo "  python3 S5_stats.py $OUT --group framework --value srv_p95_ms"
